...
public static partial class HttpClientExtension
{
    ...
    public static ValueTask<System.Net.Http.HttpResponseMessage?> SendWithRetry(this System.Net.Http.HttpClient client, string uri, int numberOfRetries = 2,
        ILogger? logger = null, TimeSpan? baseDelay = null, bool log = true, CancellationToken cancellationToken = default)
    {
        ...
        return SendWithRetry(client, request, numberOfRetries, logger, baseDelay, log, cancellationToken);
    }
    ...
    /// <param name="log"></param>
    /// <param name="cancellationToken"></param>
    public static async ValueTask<System.Net.Http.HttpResponseMessage?> SendWithRetry(this System.Net.Http.HttpClient client, HttpMethod httpMethod, string uri, object? request = null, int numberOfRetries = 2,
        ILogger? logger = null, TimeSpan? baseDelay = null, bool log = true, CancellationToken cancellationToken = default)
    {
    ...
    {
        System.Net.Http.HttpResponseMessage? response = default;

        try
        {
            response = await SendWithRetryInternal(client, request, numberOfRetries, logger, baseDelay, log, cancellationToken).NoSync();
        }
        catch (Exception ex)
        {
            logger?.LogError(ex, "Aborting SendWithRetry, exhausted max retry attempts, returning null HttpResponseMessage response");
        }

        return response;
    }

    private static async ValueTask<System.Net.Http.HttpResponseMessage> SendWithRetryInternal(this System.Net.Http.HttpClient client, System.Net.Http.HttpRequestMessage request,
